        ...
        <section id="services" className="section-anchor section-shell bg-gray-50 dark:bg-[#101010]">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div id={`service-panel-${activeCategory.id}`} role="tabpanel" aria-labelledby={`service-category-${activeCategory.id}`} tabIndex={0} className="mt-10">
                    {activeCategory.services.length > 0 ? <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{activeCategory.services.map((service) => <article key={service.id} className="service-card">
                        {service.image ? <img src={`/storage/${service.image}`} width="640" height="360" loading="lazy" decoding="async" alt={service.name} className="aspect-video w-full object-cover" /> : <div className="service-image-empty" aria-hidden="true">{activeCategory.icon}</div>}
                        <div className="p-6"><div className="flex items-start justify-between gap-4"><h3 className="text-xl font-semibold text-gray-950 dark:text-white">{service.name}</h3>{service.is_coming_soon ? <span className="status-badge"><HeroClock className="h-4 w-4" />{t('services.coming_soon')}</span> : null}</div>{service.description ? <p className="mt-3 text-sm leading-6 text-gray-600 dark:text-white/65">{service.description}</p> : null}{!service.is_coming_soon && service.link ? <a href={service.link} target="_blank" rel="noopener noreferrer" className="mt-6 inline-flex items-center gap-2 font-semibold text-primary hover:text-primary-300">{t('services.visit')}<HeroArrowTopRight className="h-4 w-4" /></a> : null}</div>
                    </article>)}</div> : <p className="section-copy">{t('services.empty')}</p>}
                </div>
            </div>
        </section>
    );
